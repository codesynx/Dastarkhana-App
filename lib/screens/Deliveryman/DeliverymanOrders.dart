...
class _DeliverymanOrdersState extends State<DeliverymanOrders> {
  ...
  // Функция для перевода статусов заказа на казахский язык
  ...
  Future<void> updateOrderStatus(int id, String status) async {
    try {
      ...
      if (response.statusCode == 200) {
        String translatedStatus = getOrderStatusTranslation(status);
        print("Тапсырыстың күйі $translatedStatus болып жаңартылды");
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Тапсырыс күйі "$translatedStatus" болып жаңартылды'),
  ...
  Widget build(BuildContext context) {
    ...
    if (_orders.isEmpty) {
      return Scaffold(
        ...
        body: Center(
          child: EmptyOrdersDelivery(), // Replaced EmptyCart with EmptyOrdersDelivery
        ),
      );
    }
                            ...
                            if (status != 'ON_ROAD') ...[
                              Expanded(
                                child: MaterialButton(
                                  ...
                                  onPressed: () {
                                    updateOrderStatus(order['id'], 'ON_ROAD');
                                  },
                                ),
                              ),
                              SizedBox(width: 10),
