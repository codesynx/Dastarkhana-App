...
class _HomescreendeliverymanState extends State<Homescreendeliveryman> {
  ...
  Future<void> updateOrderStatus(int id, String newStatus) async {
    try {
      ...
      if (response.statusCode == 200) {
        String translatedStatus = getOrderStatusTranslation(newStatus);
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Тапсырыстың статусы \"$translatedStatus\" болып жаңартылды")),
        );
      } else {
        throw Exception(
